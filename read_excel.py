...
def get_measurement(location):
    ...
    for r in range(1,row_num):
        for c in range(2,column_num):
            if c == 2:
                measurement_list[content.cell(row=r, column=1)._value] = 'netdata.%s.user' % content.cell(row=1, column=c)._value
                continue
            measurement_list[content.cell(row=r, column=1)._value] = 'netdata.%s.used' % content.cell(row=1, column=c)._value
